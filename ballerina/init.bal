...
function registerInDashboardServer() returns error? {
    ...
    while (true) {
        ...
        if (resp is http:ClientError) {
            log:printError(string `Connection to dashboard server ${dashboard.url} is failed due to http error ${resp.toString()}. Retring after ${dashboard.heartbeatInterval} seconds.`);
            isFailed = true;
        } else if (resp.statusCode != 200) {
            string|http:ClientError textPayload = resp.getTextPayload();
            if textPayload is string {
                log:printError(string `Connection to dashboard server ${dashboard.url} is failed due to ${textPayload} with error code ${resp.statusCode}. Retring after ${dashboard.heartbeatInterval} seconds.`);
            } else {
                log:printError(string `Connection to dashboard server ${dashboard.url} is failed due to client error ${textPayload.message()}. Retring after ${dashboard.heartbeatInterval} seconds.`);
            }
            isFailed = true;
        } else if (isFailed) {
